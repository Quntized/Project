...
#include "serial_graphics.h"
#include <math.h>     // <--- Fixes 'sin', 'cos', 'pow', 'tanh'
#include <stdlib.h>
...
// Wall detection
// Wall detection
#define hitBottom(b) (b > int2fix15(240))
...
void refreshStats() {
  ...
  // Cycle through the stats (0 to 6)
  if (stats_counter == 7) {
    stats_counter = 0;
  }
  else {
...
#define ENCODER2_CLK 6  
#define ENCODER2_DT  7  
#define ENCODER3_CLK 28  
#define ENCODER3_DT  22  
#define ENCODER4_CLK 8  
#define ENCODER4_DT  9  

// --- Global State Variables ---
static int last_CLK_state = 0;
static int last_CLK_state2 = 0;
static int last_CLK_state3 = 0;
static int last_CLK_state4 = 0;

// Debouncer States
int debouncer_state_button1 = 0; 
int debouncer_state_button2 = 0; 
int debouncer_state_button3 = 0; 
int debouncer_state_button4 = 0; 
...
// Button 1: Manual Mode + Max Rod Height
void read_button1() {
  int input = gpio_get(PIN_BUTTON_1);
  if (debouncer_state_button1 == 0) { 
      debouncer_state_button1 = (input == 0) ? 1 : 0;
  } else if (debouncer_state_button1 == 1) { 
      if (input == 0) {
          debouncer_state_button1 = 2;
          auto_mode = 0; // Manual
          control_rod_target_y = int2fix15(12); // Max height
          encoder_position = (12 + 105) / encoder_sensitivity;
      } else {
          debouncer_state_button1 = 0;
      }
  } else if (debouncer_state_button1 == 2) { 
...
// Button 2: Toggle Auto/Manual
void read_button2() {
  int input = gpio_get(PIN_BUTTON_2);
  if (debouncer_state_button2 == 0) { 
      debouncer_state_button2 = (input == 0) ? 1 : 0;
  } else if (debouncer_state_button2 == 1) { 
      if (input == 0) {
  ...
  } else if (debouncer_state_button2 == 2) { 
...
// Button 3: Sync Manual Mode to Current Position
void read_button3() {
  int input = gpio_get(PIN_BUTTON_3);
  if (debouncer_state_button3 == 0) { 
      debouncer_state_button3 = (input == 0) ? 1 : 0;
  } else if (debouncer_state_button3 == 1) { 
      if (input == 0) {
  ...
  } else if (debouncer_state_button3 == 2) { 
...
// Button 4: Toggle Simulation Speed
void read_button4() {
  int input = gpio_get(PIN_BUTTON_4);
  if (debouncer_state_button4 == 0) { 
      debouncer_state_button4 = (input == 0) ? 1 : 0;
  } else if (debouncer_state_button4 == 1) { 
      if (input == 0) {
          debouncer_state_button4 = 2;
          if(sim_speed_state == 1) sim_speed_state = 0; 
          else sim_speed_state = 1; 
      } else {
          debouncer_state_button4 = 0;
      }
  } else if (debouncer_state_button4 == 2) { 
...
static PT_THREAD(protothread_buttons(struct pt *pt)) {
  ...
  while (1) {
      ...
      PT_YIELD_usec(10000); // 10ms delay (100Hz polling)
  }
